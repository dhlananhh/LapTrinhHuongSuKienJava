...
public class GUI extends JFrame implements ActionListener {
	...
	private String[] listTheLoai = {"Toán", "Ngữ Văn", "Tiếng Anh", "Tin học"};
	private quanly_Sach ds;
	...
	public void loadData() throws Exception {
		...
		if (ds == null) {
			...
			for (Sach sach : ds.getList()) {
				String[] row = {sach.getMaSach(), sach.getTenSach(), sach.getSoTrang() + "", sach.getTheLoai(), sach.getNhaXB()};
				model.addRow(row);
			}
		}
	}
...
//		b.add(Box.createRigidArea(new DimensionUIResource(0, 10)));
	...
	public void createTable() {
		...
		columnTheLoai.setCellEditor(new DefaultCellEditor(cmbTheLoai));
		...
		table.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				txtSoTrang.setText((String)model.getValueAt(row, 1));
				for (int i=0; i<listTheLoai.length; i++) {
					if (cmbTheLoai.getItemAt(i).equalsIgnoreCase((String)model.getValueAt(row, 3)))
						cmbTheLoai.setSelectedIndex(i);
				}
				txtNhaXB.setText((String)model.getValueAt(row, 3));
			}
		});
	}
